	...
	if(salario <= 2112, 0, 
		if(salario > 2112 and salario <= 2826.85, format((salario * 13.3) * 0.075, 2), 
			if(salario > 2826.85 and salario <= 3751.05, format((salario * 13.3) * 0.15, 2),
				if(salario > 3751.05 and salario <= 4664.68, format((salario * 13.3) * 0.225, 2), format((salario * 13.3) * 0.275, 2))
            )
        )
    ) as Tributo
from
...
select
    ...
    concat("R$ ", format(((salario * 13.3) - (2275 * count(dep.cod_dep))) / 13.3, 2)) as IRPF_real
from
	empregado emp
left join dependente dep
on emp.cod_emp = dep.cod_dep
group by
	emp.nome,
	emp.salario